...
- **Гистерезис группировки** — решение «склеить эти строки в один блок или нет» помнится между фреймами и переиспользуется на той же сцене, чтобы OCR-jitter не дёргал склейку туда-сюда
- **Frame reuse** — если новые OCR-регионы по позиции и тексту совпадают с прошлым кадром, движок не запускает повторно весь пайплайн (OCR → группировка → перевод), а просто перерисовывает прошлый результат. Это и стабилизирует оверлей, и экономит LLM-вызовы
...
- **Чистый список окон** — в окне выбора скрыты системные оболочки (Проводник, ApplicationFrameHost), мессенджеры (Telegram), VPN-клиенты, утилиты драйверов (Razer, Logitech) и сама программа — обычно их не переводят. Чекбокс «Показать скрытые приложения» возвращает полный список если нужно
