...
- **Стриминг LLM-перевода** — текст появляется в оверлее по мере генерации (~200 мс до первых символов вместо ~1-2 сек ожидания полного ответа). Включено по умолчанию для всех LLM-провайдеров (OpenAI, Anthropic, Gemini, Groq), выключается одним чекбоксом
...
- **Маскирование API-ключей** в UI (через `PasswordBox`) с кнопкой 👁 «показать», ключи шифруются через Windows DPAPI
...
- **Умная группировка соседних строк** — многострочный диалог переводится как одно предложение. Группировка анализирует не только расстояние между строками, но и сам текст: запятая/двоеточие в конце → почти точно продолжение, точка/!/? → новое предложение. Защита от слипания меню-кнопок: однословные капс-лейблы (`NORMAL`/`HARD`/`SAVE`) остаются раздельными даже близко друг к другу
- **Стабилизация OCR-jitter** — между фреймами OCR может выдавать чуть разный bounding box или подменять `,`/`.` для одного и того же текста. Движок помнит последние ~5 фреймов и подменяет почти-идентичный новый текст на стабильную версию (Levenshtein ≤ 3 правок), чтобы кэш переводов попадал и оверлей не моргал
- **Контейнмент перевода** — ширина оверлея ограничена ~10% от ширины оригинального бокса (плюс font-aware запас), чтобы длинные переводы переносились на новую строку, а не вылезали за пределы UI игры
...
| Пауза/возобновить (опционально) | по умолчанию не задано |

Меняются в **Настройки перевода → Горячие клавиши**. Кликни по полю → нажми сочетание → готово. Esc отменяет, ✕ сбрасывает. Минимум один модификатор обязателен.

«Пауза» отличается от «Включить/выключить»: при паузе движок остаётся живым (выбранное окно, OCR, контекст LLM, статистика — всё сохраняется), просто кадры не обрабатываются. Удобно во время катсцен / звонков.
...
**Способ 2 — из истории (v1.0.14):** замечаешь в табе «История» строку с кривым переводом → правый клик по ней → «Добавить в глоссарий…» → диалог откроется с предзаполненным оригиналом и переводом, обрезаешь оба до конкретного имени → «Сохранить». Правило применится со следующего перевода.
...
PaddleOCR кэширует свои модели в `%LocalAppData%\Sdcb\`.
...
│   ├── Controls/                — MaskedSecretBox (PasswordBox + TextBox с глазиком), HotkeyCaptureBox (click-to-capture хоткеев)
│   ├── Dialogs/                 — UpdateAvailableDialog, WhatsNewDialog, AddToGlossaryDialog
